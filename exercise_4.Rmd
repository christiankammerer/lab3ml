...
### 1.
The given template was used to complete this task 
```{r}
set.seed(1234567890)
# create data
Var <- runif(500, 0, 10)
...
The overall prediction quality is good. Between roughly Var = 5 and Var = 6.5 it is a little bit off though. This might be explained by the missing training data in this interval.  

### 2. 

The linear activation function, ReLU and softplus are implemented and used for three different neural nets. For ReLU the "ifelse" construct is used, as the max()-function is not differentiable and causes an error in the training.  
...
Using ReLU as activation function results in a slightly better prediction. But from Var=4 on it is completly off.  
...
But why it converges to this exact value is not clear.
